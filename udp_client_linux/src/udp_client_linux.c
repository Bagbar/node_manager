...
int main(void)
{
	...
	do
	{
		int return_send = (int) sendto(return_socket, sendBuff, 1, 0, (struct sockaddr*) &serv_addr, serv_len);
		if (return_send < 0)
			printf("senderror:%s\n", strerror(errno));
		else
			printf("send data = %d\t send_symbol=%c\n", return_send, sendBuff[0]);

		int return_recv = (int)recvfrom(return_socket, recvBuff, 1, 0, (struct sockaddr*) &serv_addr, &serv_len);
		if (return_recv < 0)
			printf("recverror:%s\n", strerror(errno));
		else
